= Decision Logic Module (DLM) Language

A DLM module has the following general structure.
...
conditions
    <simple boolean conditions (0-order functions)>
    
rules
    <complex rules (functions), returning any data type>
...
If no model is specified, the {openehr_base_releases}[openEHR Base and Foundation types^] are assumed.
...
[source,kotlin]
----
    if systolic_blood_pressure.in_range (critical_high) then
        -- emergency
    elseif systolic_blood_pressure.in_range ({high, very_high}) then
...
A subject variable declaration of the form `identifier: Type` means that the variable cannot be null, i.e. it must always have be _attached_. A variable can be declared as _nullable_ to indicate that no value can be found for it in the external world, i.e. that it may be _undefined_. This is done via the appending of a '? symbol to the type, as in some programming languages, as per the following example.
