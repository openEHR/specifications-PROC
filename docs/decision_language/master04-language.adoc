...
In the above, the semi-strict Boolean `operator` ensures the second reference to `systolic_blood_pressure` will only occur if `systolic_blood_pressure.is_available` returns True.
...
[.tbd]
TBD: exact data structure to handle discriminator.
