...
[.text-center]
.Task Taxonomy
image::diagrams/task_taxonomy.svg[id=task_taxonomy, align="center", width=60%]
...
=== [[wait_condition]]Wait Conditions
...
* as a _relative offset_ from the start of the Plan, when it is activated;
* as an _absolute time_ as marked on an external clock or calendar;
* as the _time of an event_.

The kinds of events recognised are as follows:

* _timer expiry_: an event caused by a timer that expires;
* _calendar event_: an event generated by an entry on a calendar becoming active, according to current clock time during execution;
...
* _callback event_: an event generated by a notification received on completion of a Task dispatched to a different Plan or an external system;
* _manual event_: an external event is signalled manually to the system by a user, e.g. receipt of a phone call.

Tasks can be defined to wait on either one or more events, or on a time condition. 
