...
If `_terminate_condition_` is set, it is used _post_ each repeat iteration. It takes precedence over the `_repeats_` attribute, i.e. if the terminate condition is true, no further repeats occur, even if the total number of repeats so far is less than the lower bound specified in `_repeats_`.
