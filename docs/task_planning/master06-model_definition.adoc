...
A `PLAN_ITEM` also has a `_review_dataset_` attribute, which allows a screen form or other data set to be specified that should be provided to the user at runtime on entry to a Task or Task Group structure.
...
[.text-center]
.Task grouping
image::diagrams/task_grouping.svg[id=task_grouping, align="center", width=75%]
...
The `PERFORMABLE_TASK` class attribute `_capture_dataset_` allows a dataset or form to be specified, for which data must be obtained during the Task execution.
...
A subject precondition is formally represented as a `BOOLEAN_CONTEXT_EXPRESSION`, whose `_expression_` value is a string in {openehr_el}[openEHR Expression Language] syntax.
...
[.text-center]
.Task Plan linking
image::diagrams/task_linking.svg[id=task_linking, align="center"]
...
[.text-center]
.Task definition
image::diagrams/task_definition.svg[id=task_definition, align="center", width=65%]
...
[.text-center]
.Task coordination
image::diagrams/task_coordination.svg[id=task_coordination, align="center"]
...
[.text-center]
.Condition structure
image::diagrams/condition_structure.svg[id=condition_structure, align="center"]
...
[.text-center]
.Decision structure
image::diagrams/decision_structure.svg[id=decision_structure, align="center"]
...
[.text-center]
.Event structure
image::diagrams/event_structure.svg[id=event_structure, align="center"]
...
image::{uml_export_dir}/diagrams/PROC-TaskStateMachine.svg[id=task_state_machine, align="center", width=80%]
