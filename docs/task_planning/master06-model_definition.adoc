...
Tasks are represented by the `Task` class whose two concrete sub-types distinguish the two basic flavours of Task, namely _dispatchable_, meaning 'dispatched for execution to another performer', and _performable_, meaning 'performed by the current performer'. `TASK` is a generic (i.e. templated) type, whose generic parameter is constrained to the type `TASK_ACTION`, the latter of whose subtypes defined the particular kinds of Tasks available. The two concrete subtypes have a small number of attributes globally applicable to all Task Actions that may be attached to them.
...
|`TIMER_EVENT`
a|image::{diagrams_uri}/event_timer.svg[id=event_timer, align="center"]
|Event generated by the expiry of a Timer that was launched at some earlier time.

|`TIMELINE_MOMENT`
a|image::{diagrams_uri}/event_timeline_moment.svg[id=event_timeline_moment, align="center"]
...
|`CALENDAR_EVENT`
a|image::{diagrams_uri}/event_calendar_event.svg[id=event_calendar_event, align="center"]
...
|`TASK_TRANSITION`
a|image::{diagrams_uri}/event_task_transition.svg[id=event_task_transition, align="center"]
|Event generated by the lifecycle transition of a Task, such as transition to `cancelled` or `done`.

|`STATE_TRIGGER`
a|image::{diagrams_uri}/event_state_trigger.svg[id=event_state_trigger, align="center"]
...
|`MANUAL_NOTIFICATION`
a|image::{diagrams_uri}/event_manual_notification.svg[id=event_manual_notification, align="center"]
|An event that is manually notified to the Plan execution engine by a user.

|`SYSTEM_NOTIFICATION`
a|image::{diagrams_uri}/event_system_notification.svg[id=event_system_notification, align="center"]
|An event that is notified to the Plan execution engine by a system.

|`DELAY_EVENT`
a|image::{diagrams_uri}/event_delay.svg[id=event_delay, align="center"]
|A special kind of event is used to represent any other event with an added delay.

|`CALLBACK_NOTIFICATION`
a|image::{diagrams_uri}/event_callback.svg[id=event_callback, align="center"]
