...
The details of these structures are described in various sections below.
...
For non-deterministic Events, the `_timeout_` attribute may be set to point to a `TIMER_WAIT`, which enables timeout actions to be specified.
...
A specialisation of `EVENT_WAIT<T>` is `CALLBACK_WAIT`, used to handle notifications of dispatched Tasks that complete, as well as timeouts if no response is received. This class which adds three attributes:
...
The special Event type `DELAY_EVENT` may be used to add a delay to any other Event, e.g. to wait for a Task transition to `cancelled` plus 1 hour.
