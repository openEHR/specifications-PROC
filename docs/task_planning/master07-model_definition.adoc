...
* `_timers_`: a reference list of all Timers in use in the Plan;
* `_listeners_`: a reference list of all Event listeners defined in the Plan.

The details of all these structures are described in various sections below.
...
`PLAN_ITEM` has a mandatory `_description_` attribute, which represents a natural language specification of the work of the Task. It also has two optional attributes that control the generic behaviour of Tasks and Groups: `_wait_spec_` and `_repeat_spec_`. The first enables a wait state (described above in the section <<wait_condition,Wait Conditions>>) to be applied to a Task or Group, which is triggered by time-related Events (clock time, reaching a point in a calendar) or other kinds of Events (external notifications etc). This allows the timing of a Task to be specified.
...
[.text-center]
.Task coordination
image::diagrams/task_coordination.svg[id=task_coordination, align="center", width=60%]

In this Work Plan, three Task Plans are used to perform (parts of) the clinical work coordinated for managing an acute stroke, as per a Care Pathway. There are two Hand-offs, the first synchronous (`_wait_` = `True`; callback listener resumes at the next Task) and the second an asynchronous fork (`_wait_` = `False`).
...
The second structure corresponds to a decision point in a workflow at which some expression is evaluated, and each outgoing branch corresponds to a sub-range of the expression's value range. The classes `DECISION_GROUP` and `DECISION_BRANCH` provide this structure, which is  equivalent to a switch statement in a programming language. The branch sub-ranges should be individually mutually exclusive, and collectively they should cover the entire value range of the expression. If these criteria are not met, the Decision Group may not be decidable, e.g. if the tested variable is outside the range of every branch. There is no implied ordering of processing the branches, so `_execution_type_` is constrained to `parallel`.
...
In the above, the `PLAN_EVENT` class is sub-typed to represent two categories of events: _deterministic events_, i.e. known, system-endogenous events, and _non-deterministic events_, i.e. events from the outside world whose receipt is not guaranteed.

Three deterministic event types are included as follows:

* `TIMER_EXPIRY`: represents the expiry of a Timer that was launched at some earlier time;
* `TIMELINE_MOMENT`: represents a time-point within the global Plan timeline that is specified _relative_ to the Plan's start time, and converted to absolute time at activation;
...
* `MANUAL_EVENT`: an event that is manually signalled to the Plan execution engine by a user.

Instances of all of these types, on their own only identify the type and source of an event. To intercept events and subsequently perform actions requires an `EVENT_LISTENER`, which is a generic class from which any concrete type may be constructed based on the type of Event to be listened for. Types such as `EVENT_LISTENER <MANUAL_EVENT>` may thus be constructed. The `EVENT_LISTENER` class has a `_success_action_` attribute of type `EVENT_ACTION`, which enables any of the following actions to occur on receipt of an event:
...
* starting a named Timer (`_timer_` attribute).

A more specialised kind of Listener is `ND_EVENT_LISTENER`, which is based on the generic type `EVENT_LISTENER <NON_DETERMINISTIC_EVENT>`, adds a `_timeout_`, which enables a `TASK_TIMER` to be launched, which in turn will result in a `TIMER_EXPIRY` event for which a another `EVENT_LISTENER` may be configured. This would normally only be applied to non-deterministic events, ensuring that non-arrival does not block Plan execution.

A further specialisation of `ND_EVENT_LISTENER` is `CALLBACK_LISTENER`, which adds three attributes:

* `_fail_action_`: enables a different Action to be specified on receipt of a callback with a 'fail' status;
* `_resume_type_` and `_resume_location_`: these enable resumption of processing at a different location in the Plan, e.g. an earlier Task that may need to be redone.

=== Specifying Task Timing

Time is manifested in the model in a number of ways. Firstly, there are three kinds of events that encode time directly, as described above. Of these, the `TIMELINE_MOMENT` type corresponds to what is normally understood as 'clock time' or 'diary' time in the sense of a modern calendar or diary tool. This concept is called the _timeline_ here to distinguish it from the global Plan _calendar_, which contains events already fixed in time, e.g. appointments made by other systems, holidays etc. 

Any Plan Item (i.e. `TASK`, `TASK_GROUP` or descendant) may specify timing via its `_wait_spec_` attribute, which refers to an instance of `TASK_WAIT`. The `TASK_WAIT` class models a wait state, and defines how to specify the details of when a Task should start with respect to any of the Event types described above. Its `_events_` attribute enables more than multiple Events to be used as a trigger, with an assumed logical OR relation among the Events. This enables the specification of triggers such as 'at 8pm on day 1, or when oxygen saturation drops below 90% (whichever comes first)'. The `_event_relation_` attribute allows the Task to be specified as commencing `before`, `with` or `after` the trigger event (such as a meal). The `_start_window_` attribute allows for a delay from the trigger moment, and  `_timeout_`, if set, will cause a Timer to be started in cases where Event receipt is not certain.
...
* _event-linked time_: the Task is to be executed at a time related to a real world event, such as a meal or sleeping; in this case, an instance of `CALENDAR_EVENT` or `MANUAL_EVENT` is used; the `TASK_WAIT` attributes indicate the specifics such as '1 hour before', 'with meal' etc;
...
The `TIMELINE_MOMENT` type allows a point in time relative to the start of the Plan to be specified, in terms of fixed and variable components. The variable part is understood as an offset from the Plan zero point at activation, while the fixed part enables fixed points in time such as a particular hour or day to be specified. A combination of the two such as +1d, 01:30:00 can thus be used to state a time like '1:30 PM on day one of the plan'.

=== Timers

The type `TIMER_EXPIRY` corresponds to the expiry of a Timer that has been launched at some earlier point in time. `TIMER` and `TIMER_EXPIRY` instances thus occur in pairs.


=== Callbacks

Using the above scheme, system callbacks can be conveniently represented with a `CALLBACK_LISTENER`.
...
include::{uml_export_dir}/classes/task_wait.adoc[]
...
include::{uml_export_dir}/classes/plan_event.adoc[]
...
include::{uml_export_dir}/classes/callback_notification.adoc[]
...
include::{uml_export_dir}/classes/timeline_moment.adoc[]

include::{uml_export_dir}/classes/task_timer.adoc[]

include::{uml_export_dir}/classes/event_listener.adoc[]

include::{uml_export_dir}/classes/nd_event_listener.adoc[]

include::{uml_export_dir}/classes/callback_listener.adoc[]
