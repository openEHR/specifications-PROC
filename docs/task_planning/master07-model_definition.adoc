...
* optionally indicates where execution should resume in the plan, for example at an earlier Task, via the `_resume_type_` and `_resume_location_` attributes.

The latter attributes are normally only for use with callbacks.
