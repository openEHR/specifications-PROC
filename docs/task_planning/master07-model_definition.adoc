...
[.text-center]
.Task coordination
image::diagrams/task_coordination.svg[id=task_coordination, align="center", width=65%]
...
Three event types are as follows:
...
Instances of all of these types, on their own only identify the type and source of an event. Events can be waited on in two ways. The first is via the `TASK_WAIT` attachable to any Plan Item, via the `_wait_spec_` attribute. This represents a timing wait-state for a Task and is described in detail below.

The other way to intercept events and subsequently perform actions is via an instance of `EVENT_LISTENER<T>`, which is a generic class from which concrete types may be constructed based on the type of Event to be listened for. Types such as `EVENT_LISTENER <MANUAL_EVENT>` may thus be constructed. The `EVENT_LISTENER` class has a `_success_action_` attribute of type `EVENT_ACTION`, which enables any of the following actions to occur on receipt of an event:
...
For non-deterministic Events, the `_timeout_` attribute may be set, which enables a `TASK_TIMER` to be launched, which in turn will result in a `TIMER_EXPIRY` event for which a another `EVENT_LISTENER` may be configured. This would normally only be applied to non-deterministic events, ensuring that non-arrival does not block Plan execution.

A specialisation of `EVENT_LISTENER<T>` is `CALLBACK_LISTENER`, which adds three attributes:
...
Timers are used in various places in the model to enable time delays or wait periods to be specified relative to another event in time, when it occurs. The type `TASK_TIMER` is used to define a Timer, and the Event sub-type `TIMER_EXPIRY` is sed to represent to its expiry. Timers have an identifier which enables a Timer Expiry instance to refer to its launching Timer. When a Timer is launched at execution time, a matching `TIMER_EXPIRY` attached to either `PLAN_ITEM._wait_spec_` or to an `EVENT_LISTENER` attached elsewhere in the model will enable the expiry event to be caught and acted upon.
...
In addition to timing and events, other conditions may determine when or if a Task can be performed. One type is modelled as the `TASK_ACTION` attribute `_subject_preconditions_: TASK_PRECONDITION`. A subject precondition is formally represented as a `BOOLEAN_CONTEXT_EXPRESSION`, whose `_expression_` value is a string in {openehr_expression}[openEHR Expression Language] syntax.
